...
import { INITIAL_LOT_SERIALS } from '../data/initialData';
...
export const InventoryView: React.FC<InventoryViewProps> = ({
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {/* Modal: Lot & Serial Number Traceability */}
      {showLotModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs">
          <div className="w-full max-w-3xl bg-white border border-slate-200 rounded-2xl shadow-2xl p-6 relative space-y-4">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              <div>
                <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                  <Boxes className="w-5 h-5 text-indigo-600" />
                  <span>Genealogy & Lot / Serial Number Traceability</span>
                </h3>
                <p className="text-xs text-slate-500">
                  Full lineage tracking for high-value components, expiration dates, and warehouse locations.
                </p>
              </div>
              <button
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs text-slate-600">
                ...
                <tbody className="divide-y divide-slate-100 font-mono">
                  {INITIAL_LOT_SERIALS.map((ls) => (
                    <tr key={ls.id} className="hover:bg-slate-50">
                      <td className="p-2.5 font-sans">
                        <span className="font-bold text-slate-900 block">{ls.sku}</span>
                        <span className="text-[11px] text-slate-500">{ls.itemName}</span>
                      </td>
                      <td className="p-2.5 text-indigo-600 font-bold">{ls.lotNumber}</td>
                      <td className="p-2.5 font-bold text-slate-800">{ls.serialNumber}</td>
                      <td className="p-2.5">{ls.manufacturingDate}</td>
                      <td className="p-2.5">{ls.expirationDate}</td>
                      <td className="p-2.5 font-sans text-[11px] text-slate-600">{ls.warehouseLocation}</td>
                      <td className="p-2.5 font-sans">

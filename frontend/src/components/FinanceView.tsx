...
import { INITIAL_ENTITIES } from '../data/initialData';
...
export const FinanceView: React.FC<FinanceViewProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'gateways' | 'invoices' | 'ai_cfo_suite' | 'multi_entity'>('overview');
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5">
          <div className="flex items-center bg-slate-100 border border-slate-200 rounded-xl p-1 text-xs font-medium">
            ...
            <button
              onClick={() => setActiveTab('multi_entity')}
              className={`px-3 py-1.5 rounded-lg transition-colors flex items-center space-x-1.5 ${
                activeTab === 'multi_entity' ? 'bg-white text-indigo-700 font-semibold shadow-2xs' : 'text-slate-600 hover:text-slate-900'
              }`}
            >
      ...
      {/* MULTI-ENTITY TREASURY CONSOLIDATION TAB */}
      {activeTab === 'multi_entity' && (
        <div className="space-y-6">
          <div className="p-6 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
              <div>
                <h3 className="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
                  ...
                  <span>Multi-Entity General Ledger & Real-Time Currency FX Consolidation</span>
                </h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                  Automated FX rate translation (USD base currency) for international subsidiaries across North America, EMEA, and APAC.
                </p>
              </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs text-slate-600 dark:text-slate-300">
                <thead className="bg-slate-50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-200 font-semibold border-b border-slate-200 dark:border-slate-800">
                  <tr>
                    <th className="p-3">Entity Name</th>
                    <th className="p-3">Region</th>
                    <th className="p-3">Base Currency</th>
                    <th className="p-3">FX Rate (to USD)</th>
                    <th className="p-3 text-right">Local Revenue</th>
                    <th className="p-3 text-right">Consolidated USD Revenue</th>
                    <th className="p-3 text-right">Local OpEx</th>
                    <th className="p-3 text-right">Consolidated USD OpEx</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                  {INITIAL_ENTITIES.map((ent) => (
                    <tr key={ent.entityId} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 font-mono">
                      <td className="p-3 font-sans font-semibold text-slate-900 dark:text-white flex items-center gap-2">
                        <Building2 className="w-4 h-4 text-indigo-600 shrink-0" />
                        {ent.entityName}
                      </td>
                      <td className="p-3 font-sans">{ent.region}</td>
                      <td className="p-3">
                        <span className="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 font-bold text-slate-800 dark:text-slate-200">
                          {ent.baseCurrency}
                        </span>
                      </td>
                      <td className="p-3 text-indigo-600 font-bold">{ent.exchangeRateToUSD.toFixed(3)}</td>
                      <td className="p-3 text-right font-bold">
                        {ent.baseCurrency === 'USD' ? '$' : ent.baseCurrency === 'EUR' ? '€' : 'S$'}{ent.localRevenue.toLocaleString()}
                      </td>
                      <td className="p-3 text-right text-emerald-600 font-bold">
                        ${ent.consolidatedRevenueUSD.toLocaleString()}
                      </td>
                      <td className="p-3 text-right">
                        {ent.baseCurrency === 'USD' ? '$' : ent.baseCurrency === 'EUR' ? '€' : 'S$'}{ent.localOperatingExpenses.toLocaleString()}
                      </td>
                      <td className="p-3 text-right font-semibold">
                        ${ent.consolidatedOpExUSD.toLocaleString()}
                      </td>
                    </tr>
                  ))}
                </tbody>
